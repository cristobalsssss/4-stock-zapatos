-- ==============================================================================
-- SCHEMA COMPLETO - Stock Zapatos (Ejecutar en Supabase SQL Editor)
-- Versión optimizada para ejecución via pooler pg8000 (transacción por bloque)
-- ==============================================================================
...
CREATE TABLE IF NOT EXISTS public.historial_precios (
    ...
    motivo_cambio TEXT DEFAULT 'Actualizacion de tarifa',
...
RETURNS TRIGGER AS $fn_updated$
BEGIN
    NEW.updated_at = timezone('utc'::text, now());
    RETURN NEW;
END;
$fn_updated$ LANGUAGE plpgsql;
    ...
    IF (OLD.precio_interno IS DISTINCT FROM NEW.precio_interno) OR
       (OLD.precio_vendedores IS DISTINCT FROM NEW.precio_vendedores) THEN
        INSERT INTO public.historial_precios (
            ...
            'Modificacion de precio en mantenedor',
            COALESCE(current_setting('app.current_user', true), 'admin')
        );
    END IF;
    RETURN NEW;
END;
$fn_precio$ LANGUAGE plpgsql;
...
SELECT
    ...
    json_agg(DISTINCT jsonb_build_object('color', iv.color, 'imagen_portada', iv.imagen_portada_variante)) FILTER (WHERE iv.id IS NOT NULL) AS colores,
    json_agg(DISTINCT iv.talla) FILTER (WHERE iv.id IS NOT NULL) AS tallas_disponibles,
...
DROP POLICY IF EXISTS "Lectura publica de inventario" ON public.inventario_variantes;
CREATE POLICY "Lectura publica de inventario" ON public.inventario_variantes FOR SELECT USING (true);

DROP POLICY IF EXISTS "Lectura publica de imagenes" ON public.imagenes_variante;
CREATE POLICY "Lectura publica de imagenes" ON public.imagenes_variante FOR SELECT USING (true);

DROP POLICY IF EXISTS "Lectura publica de ventas" ON public.ventas;
CREATE POLICY "Lectura publica de ventas" ON public.ventas FOR SELECT USING (true);

DROP POLICY IF EXISTS "Lectura publica de detalle_movimientos" ON public.detalle_movimientos;
CREATE POLICY "Lectura publica de detalle_movimientos" ON public.detalle_movimientos FOR SELECT USING (true);

DROP POLICY IF EXISTS "Lectura publica de historial_precios" ON public.historial_precios;
CREATE POLICY "Lectura publica de historial_precios" ON public.historial_precios FOR SELECT USING (true);

DROP POLICY IF EXISTS "Lectura publica de log_busquedas" ON public.log_busquedas_vistas;
CREATE POLICY "Lectura publica de log_busquedas" ON public.log_busquedas_vistas FOR SELECT USING (true);

DROP POLICY IF EXISTS "Escritura productos" ON public.productos;
CREATE POLICY "Escritura productos" ON public.productos FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Escritura inventario" ON public.inventario_variantes;
CREATE POLICY "Escritura inventario" ON public.inventario_variantes FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Escritura imagenes" ON public.imagenes_variante;
CREATE POLICY "Escritura imagenes" ON public.imagenes_variante FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Escritura ventas" ON public.ventas;
CREATE POLICY "Escritura ventas" ON public.ventas FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Escritura detalle_movimientos" ON public.detalle_movimientos;
CREATE POLICY "Escritura detalle_movimientos" ON public.detalle_movimientos FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Escritura log" ON public.log_busquedas_vistas;
CREATE POLICY "Escritura log" ON public.log_busquedas_vistas FOR ALL USING (true) WITH CHECK (true);

-- 7. STORAGE POLICIES
INSERT INTO storage.buckets (id, name, public)
VALUES ('productos', 'productos', true), ('productos-imagenes', 'productos-imagenes', true)
ON CONFLICT (id) DO UPDATE SET public = true;

DROP POLICY IF EXISTS "Storage lectura publica" ON storage.objects;
CREATE POLICY "Storage lectura publica" ON storage.objects FOR SELECT USING (bucket_id IN ('productos', 'productos-imagenes'));

DROP POLICY IF EXISTS "Storage subida" ON storage.objects;
CREATE POLICY "Storage subida" ON storage.objects FOR INSERT WITH CHECK (bucket_id IN ('productos', 'productos-imagenes'));

DROP POLICY IF EXISTS "Storage actualizacion" ON storage.objects;
CREATE POLICY "Storage actualizacion" ON storage.objects FOR UPDATE USING (bucket_id IN ('productos', 'productos-imagenes'));

DROP POLICY IF EXISTS "Storage borrado" ON storage.objects;
CREATE POLICY "Storage borrado" ON storage.objects FOR DELETE USING (bucket_id IN ('productos', 'productos-imagenes'));
