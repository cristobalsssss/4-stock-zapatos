...
2. **Ruta Privada (`/admin`):**
   ...
   - **Flujo Atómico de Conversión sin Duplicación (Revisión #15):** Al presionar "Convertir a Venta", se asocia el ID de la reserva. Al confirmar la venta, se realiza un `UPDATE` único sobre el registro existente en Supabase y localStorage cambiando su estado a "Completada", sin clonar filas ni generar registros duplicados ("SIN-CODIGO"). El contador de "Reservas Activas" se actualiza de inmediato reduciéndose a 0 de forma reactiva.
   - **Pestaña "📋 Reservas" (Nombre Fantasía + Código #RES-XXXX):** En la columna "Pares / Detalles", muestra el nombre comercial del modelo junto al código (ej: `"• Viena (AA0005) - Negro, T37 x 1"`), leídas en tiempo real con deduplicación canónica por `codigo_reserva` e `id`.
   - **Pestaña "Detalle de Movimientos" (ID Venta, Copiar y Acción "🔄 Devolver" - Revisión #15):** Muestra columna visible con el ID único de venta (`#VTA-XXXX`), botón de 1 clic para "Copiar ID" al portapapeles con confirmación visual ("¡Copiado!"), y botón de acción rápida **"🔄 Devolver"** en cada fila de venta que precarga automáticamente la variante, cantidad, ID de venta y motivo en el módulo de devoluciones.
   ...
   - **Módulo de Devoluciones:** Selector de variante y cantidad a reintegrar con registro de motivo y soporte de precarga automática desde el Kardex.
