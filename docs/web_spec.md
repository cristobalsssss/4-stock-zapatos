...
2. **Ruta Privada (`/admin`):**
   ...
   - **Pestaña "📋 Reservas" (Código #RES-XXXX y Blindaje):** Registro y trazabilidad de solicitudes de clientes con columna dedicada para el código `#RES-XXXX`, leídas en tiempo real desde Supabase y localStorage sincronizado con eventos automáticos (`reservas_updated`), con filtros por estado (Pendiente, Completada, Cancelada) y acción de **"Convertir a Venta"** que precarga automáticamente los pares en la terminal de cobro y descuento de inventario.
   ...
   - **Pestaña "⚙️ Parámetros" (Centralizada y Reactiva):** Configuración persistente del teléfono WhatsApp oficial de ventas, nombre de la vendedora de contacto, modalidad de la tienda, entregas locales y envíos nacionales con alerta visual si el número no está configurado.
