...
2. **Ruta Privada (`/admin`):**
   ...
   - **Flujo Infalible de Conversión "Convertir a Venta" (Revisión #17):** Al presionar "Convertir a Venta" en una reserva, `handleConvertirReservaAVenta` extrae los items usando `renderDetalleReserva`, empareja la variante en catálogo o asigna un fallback válido, precarga automáticamente `saleItems` con precio sugerido/acordado, asigna las notas con nombre y teléfono del cliente y asocia `convertingReservaId`. Al presionar "Confirmar Venta", se ejecuta el `UPDATE` atómico completando la reserva en Supabase.
   - **Pestaña "📋 Reservas" con Parser Universal Blindado (Revisión #17):** Procesamiento universal de items mediante `renderDetalleReserva(reserva, catalogo)` inspeccionando en orden `items`, `detalles`, `productos`, `pares_reserva`, `metadata` y fallbacks raíz. Garantiza formato estandarizado:
     `• [Nombre Fantasía] ([Código]) - [Color], T[Talla] x [Cant]`
     asegurando que ninguna fila quede vacía.
