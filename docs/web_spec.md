...
2. **Ruta Privada (`/admin`):**
   ...
   - **Reactividad Inmediata (Cero F5 - Revisión #16.1):** Al ejecutar ventas, conversiones de reservas, devoluciones o purgas, el sistema invoca de forma atómica y en paralelo `refreshAllAdminData()`, sincronizando KPIs, stock del catálogo, reservas y movimientos en tiempo real sin requerir recargar la página.
   ...
   - **Pestaña "📋 Reservas" Blindada (Revisión #16.1):** Procesamiento universal de items mediante `parseReservaItems` (soportando JSON string, arreglos estructurados y texto plano). En la columna "Pares / Detalles", muestra la lista estructurada con viñeta (`• [Nombre] ([Código]) - [Color], T[Talla] x [Cant]`) tanto para compras desde la Bolsa como desde el Chatbot, acompañada de un badge sutil de entrega (`📍 Concepción / Presencial`) sin duplicar textos de modalidad.
