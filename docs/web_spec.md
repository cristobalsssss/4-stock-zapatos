...
1. **Ruta Pública (`/`):**
   ...
   - **Bolsa de Reserva & WhatsApp Directo:** Drawer de reserva donde el cliente añade pares seleccionados, ingresa su Nombre, Teléfono WhatsApp (con prefijo pre-llenado `+56 9 ` para autocompletar 8 dígitos) y Comuna/Ciudad, seleccionando modalidad (Entrega presencial en Concepción/Penco o Envío Starken Por Pagar). Al enviar, **persiste de forma asíncrona la reserva en Supabase (`crearReserva`)** y abre `wa.me` hacia el número y nombre oficial configurado dinámicamente.
   - **Widget de Chatbot Asistente y Tarjetas Visuales (Revisión #11):** Chatbot flotante interactivo en la esquina inferior derecha (`/`), contextualizado sobre la tienda 100% online de remate de bodega.
     * **Manejo Inteligente de Quiebre de Stock:** Si el modelo/talla consultado no tiene stock disponible, emite mensaje obligatorio de apertura: *"Lamentablemente el [Modelo] en talla [Talla] está agotado por remate de bodega"*, y despliega **tarjetas de producto visuales interactivas** con prioridad (1: mismo modelo en otros colores con stock; 2: otros modelos disponibles en la misma talla).
     * **Tarjetas Visuales en Chat:** Incluyen miniatura de foto, código, nombre, color, talla, precio y botones de acción ("Ver en Catálogo" para scroll/apertura de ficha y "Reservar" para flujo conversacional embebido).
     * **Reserva Conversacional Embebida:** Formulario directo en el chat que guarda la reserva en Supabase y deriva a WhatsApp.
...
2. **Ruta Privada (`/admin`):**
   ...
   - **Pestaña "📋 Reservas" (Persistencia Supabase):** Registro y trazabilidad de solicitudes de clientes de la web y chatbot, con filtros por estado (Pendiente, Completada, Cancelada) y acción de **"Convertir a Venta"** que precarga automáticamente los pares en la terminal de cobro y descuento de inventario.
   ...
   - **Pestaña "⚙️ Parámetros" (Centralizada):** Configuración persistente del teléfono WhatsApp oficial de ventas, nombre de la vendedora de contacto (Carmen), modalidad de la tienda, entregas locales y envíos nacionales, consumidos dinámicamente por toda la aplicación sin números en duro.
